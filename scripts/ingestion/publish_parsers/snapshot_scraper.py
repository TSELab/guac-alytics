...
def get_snapshot_bydate(date, ARCH):
    for DFSG in ["main", "contrib", "non-free"]:
        if exists(f"./ingestion/publish_parsers/Packagelist_DUMP/{date}-{ARCH}-{DFSG}_Packages"):
        ...
        while True:
            ...
            if resp.status_code == 200:
                ...
                with open(f"./ingestion/publish_parsers/Packagelist_DUMP/{date}-{ARCH}-{DFSG}_Packages", "w") as f:
