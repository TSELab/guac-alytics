...
def insert_into_md(top_nodes):
    myTable = PrettyTable(["Sno","Top 5000 valued packages", "Eigenvector Centrality"])
    i=1
    ...
    with open('plots/eigen_centrality.md', 'w') as f:
            f.write(data)
...
def get_top_nodes(items):
    ...
    top_nodes = sorted(eigen.items(), key=lambda x: x[1], reverse=True)[:5000]
    return top_nodes
